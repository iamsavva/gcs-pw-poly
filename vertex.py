...
class Vertex:
    def __init__(
        ...
        self.define_potential(prog, pot_type, Q_cost)
    ...
    def get_state_potential(self, solution: MathematicalProgramResult):
        Q, r, s = (
            Evaluate(solution.GetSolution(self.Q)),
            Evaluate(solution.GetSolution(self.r)),
            Evaluate(solution.GetSolution(self.s)),
        )
        k = self.state_dim
        return Q[:k, :k], r[:k], s


    def define_potential(self, prog: MathematicalProgram, pot_type: str, Q_cost = None):
        # P =
        #   [ P_11  P_1  ]
        #   [ P_x1  Pxx  ]
        #   =
        ...
        if pot_type == PSD_ON_STATE:
            ...
            r = prog.NewContinuousVariables(self.state_dim, "r_" + self.name).reshape(
                self.state_dim, 1
            )
            ...
            self.r = prog.NewContinuousVariables(self.n, "r_" + self.name).reshape(
                self.n, 1
            )
    ...
    def cost_at_point(self, x: npt.NDArray, solution: MathematicalProgramResult = None):
        ...
        if solution is None:
            ...
            Q, r, s = (
                solution.GetSolution(self.Q),
                solution.GetSolution(self.r),
                solution.GetSolution(self.s),
            )
    ...
    def get_quadratic_potential_matrix(self):
        ...
        r = self.r
        s = self.s
        return np.vstack((np.hstack((s, r.T)), np.hstack((r, Q))))    
...
class PolytopeVertex(Vertex):
    def __init__(
        ...
        m, n = A.shape
        assert m == len(b)
        super(PolytopeVertex, self).__init__(name, prog, n, pot_type, state_dim, x_star, Q_cost)
        self.A = A  # m x n
...
class BoxVertex(PolytopeVertex):
    def __init__(
        ...
        assert n == len(ub)
        # super
        super(BoxVertex, self).__init__(name, prog, A, b, pot_type, state_dim, x_star, Q_cost)
        # define center
    ...
    def is_state_inside(self, state: npt.NDArray):
        ...
        return np.all(self.lb[: self.state_dim] <= state) and np.all(
            state <= self.ub[: self.state_dim]
        )
    
    def get_box_intersection(self, v:"BoxVertex"):
        # make new lb ub matrices
        lb = np.max((self.lb, v.lb), axis=0)
        ub = np.min((self.ub, v.ub), axis=0)
        assert np.all(ub - lb >= 0), ERROR(
            "trying to intersect boxes but they don't intersect", self.name, v.name
        )
        return lb, ub
    
    def get_control_bounds(self):
        return self.lb[self.state_dim:], self.ub[self.state_dim:]
        

    def make_box_intersection_multiplier_terms(
        self, v: "BoxVertex", lambda_e=npt.NDArray
    ):
        lb,ub = self.get_box_intersection(v)
        ...
        for i in range(self.m):
            a, bi = A[i].reshape(self.n, 1), b[i].reshape(1, 1)
...
class EllipsoidVertex(Vertex):
    def __init__(
        ...
        super(EllipsoidVertex, self).__init__(
            name, prog, n, pot_type, state_dim, x_star
        )
        # {Bu + center | |u|_2 <= 1}
        self.B = B
    ...
    def make_multiplier_terms(self, lambda_e: npt.NDArray, left: bool = None):
        ...
        if left is None:
            m_mat = np.vstack(
                (np.hstack((c.T @ G @ c - 1, -c.T @ G)), np.hstack((-G @ c, G)))
            )
        elif left:
            m_mat = np.vstack(
                (
