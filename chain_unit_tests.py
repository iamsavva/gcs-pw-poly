...
def test_simple_box_chain(n: int = 2, num_vertices: int = 2, verbose=True):
    ...
    vs = BoxVertex(
        "s", prog, (num_vertices - 1) * np.ones(n), (num_vertices) * np.ones(n)
    )
    vertices.append(vs)
    ...
    INFO(
        "Testing for a chain of "
        + str(num_vertices)
        + " consecutive boxes in "
        + str(n)
        + "-dimensional space.",
        verbose=verbose,
    )
    assert solution.is_success(), ERROR(
        "Box solve failed! ", "n: ", n, "num_vertices: ", num_vertices
    )
    assert np.allclose(-solution.get_optimal_cost(), true_cost), ERROR(
        "Cost should be", true_cost, " is ", -solution.get_optimal_cost()
    )
...
def test_simple_ellipsoid_chain(n: int = 2, num_vertices: int = 2, verbose=True):
    ...
    for i in range(num_vertices - 2):
        vv = EllipsoidVertex(
            "v" + str(i), prog, 0.5 + (i + 1) * np.ones(n), 0.5 * np.eye(n)
        )
        vertices.append(vv)

    vs = EllipsoidVertex(
        "s", prog, 0.5 + (num_vertices - 1) * np.ones(n), 0.5 * np.eye(n)
    )
    vertices.append(vs)
    ...
    INFO(
        "Testing for a chain of "
        + str(num_vertices)
        + " consecutive ellipsoids in "
        + str(n)
        + "-dimensional space.",
        verbose=verbose,
    )
    assert solution.is_success(), ERROR(
        "Ellipsoid solve failed! ", "n: ", n, "num_vertices: ", num_vertices
    )
    assert np.allclose(-solution.get_optimal_cost(), true_cost), ERROR(
        "Cost should be", true_cost, " is ", -solution.get_optimal_cost()
    )
...
def make_simple_chain_point_to_point_test():
    for n in range(1, 5):
